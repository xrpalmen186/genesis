...
class Tarea:
    def __init__(self, titulo, descripcion="", prioridad="Media", fecha_vencimiento=None, completada=False):
        # guarda los atributos — aplica normalización mínima si quieres
        self.titulo = titulo
    ...
    def mostrar_informacion(self, imprimir=True):
        completada_symbol = "X" if self.completada else " "
        ...
        info = (
            ...
            f"Vencimiento: {fecha}\n"
            f"Completada: [{completada_symbol}]"
            "\n----------------------"
        )
        if imprimir:
            print(info)
    ...
    def to_dict(self):
        # opcional ahora, útil para JSON después:
        # devuelve un dict serializable con los atributos
        return {
            "titulo": self.titulo,
    ...
    def __str__(self):
        # cadena más corta para print(t)
        fecha = self.fecha_vencimiento if self.fecha_vencimiento is not None else "Sin fecha"
        return f"{self.titulo} - {self.prioridad} ({fecha}) [{'X' if self.completada else ' '}]"
...
class GestorTareas:
    ...
    def eliminar_tarea(self, titulo):
        ...
        if not t:
            print(f"⚠️ No existe la tarea '{titulo}'")
            return False
    ...
    def listar_tareas(self):
        ...
        # Ordenar primero por estado (False primero), luego por prioridad
        tareas_ordenadas = sorted(self.tareas, key=lambda t: (t.completada, PRIORIDAD_VALOR.get(t.prioridad, 100)))
    ...
    def listar_por_estado(self, completadas=True):
        for t in self.tareas:
    ...
    def guardar_tareas(self, archivo="tareas.json"):
        ...
        if not self.tareas:
            print("⚠️ No hay tareas para guardar.")
            return
        else:
            try:
